...
if (-not $ffmpegInstalled) {
    ...
    Write-Host ""
    Write-Host "After installing, restart your terminal and run this script again." -ForegroundColor Green
    exit 1
}
...
if ($mp3Files.Count -eq 0) {
    Write-Host "ERROR: No MP3 files found in public folder!" -ForegroundColor Red
    exit 1
}
...
foreach ($file in $mp3Files) {
    ...
    # Compress to 96kbps stereo
    $result = & ffmpeg -i $inputPath -codec:a libmp3lame -b:a 96k -ac 2 -ar 44100 -q:a 2 -y $tempOutput 2>&1
    ...
    if ($LASTEXITCODE -eq 0 -and (Test-Path $tempOutput)) {
        ...
        $origSize = [math]::Round($originalFileSize, 2)
        $newSize = [math]::Round($newFileSize, 2)
        $savePct = [math]::Round($savings, 1)
        Write-Host "  Compressed: $origSize MB to $newSize MB (saved $savePct percent)" -ForegroundColor Green
        $compressed++
    } else {
...
Write-Host "Total saved: $([math]::Round($totalSavings, 2)) MB ($([math]::Round($savingsPercent, 1)) percent)" -ForegroundColor Yellow
Write-Host ""
