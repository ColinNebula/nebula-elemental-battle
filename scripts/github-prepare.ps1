...
if (Test-Path "public") {
    ...
    if ($publicSize -gt 100) {
        Write-Host " ⚠️  LARGE ($publicSizeRounded MB)" -ForegroundColor Yellow
        $WarningCount++
    } else {
...
if ($largeFiles) {
    Write-Host " ⚠️  FOUND $($largeFiles.Count)" -ForegroundColor Yellow
...
if (-not $SkipOptimization) {
    ...
    if ($unoptimizedImages) {
        Write-Host " ⚠️  FOUND UNOPTIMIZED" -ForegroundColor Yellow
    ...
    if ($unoptimizedAudio) {
        Write-Host " ⚠️  FOUND UNCOMPRESSED" -ForegroundColor Yellow
...
if (Test-Path "README.md") {
    ...
    if ($readmeSize -lt 500) {
        Write-Host " ⚠️  TOO SHORT" -ForegroundColor Yellow
        $WarningCount++
    } else {
...
# 7. BUILD TEST
# ============================================
Write-Host "🏗️  BUILD VERIFICATION" -ForegroundColor Yellow
Write-Host "─────────────────────────────────────────────" -ForegroundColor Gray
...
Write-Host "⏱️  Duration: $DurationSeconds seconds" -ForegroundColor Gray
Write-Host ""
